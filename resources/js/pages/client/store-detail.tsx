...
import { ArrowLeft, Plus, ShoppingCart, Star, Clock, Bike, Package } from 'lucide-react';
import { useState } from 'react';
import ClientLayout from '@/layouts/ClientLayout';

// Datos estáticos temporales — luego vendrán de Laravel
const stores = [
...
export default function ClientStoreDetail() {
  const { url } = usePage();
  // Extraer el ID de la URL — /client/store/1 → '1'
                    ...
                    <StaggerItem key={product.id}>
                      <div className="rounded-2xl border border-border bg-card p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-bold text-violet-600 mt-0.5">${product.price.toLocaleString()}</p>
                        </div>
                        {product.inStock ? (
                          <button
                          ...
                          >
                            {cart[product.id] ? (
                              <span className="text-white text-xs font-bold">{cart[product.id]}</span>
                            ) : (
